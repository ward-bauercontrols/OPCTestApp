namespace OPCTestApp
{
   partial class ReadingControl
   {
      /// <summary> 
      /// Required designer variable.
      /// </summary>
      private System.ComponentModel.IContainer components = null;

      /// <summary> 
      /// Clean up any resources being used.
      /// </summary>
      /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
      protected override void Dispose(bool disposing)
      {
         if (disposing && (components != null))
         {
            components.Dispose();
         }
         base.Dispose(disposing);
      }

      #region Component Designer generated code

      /// <summary> 
      /// Required method for Designer support - do not modify 
      /// the contents of this method with the code editor.
      /// </summary>
      private void InitializeComponent()
      {
         lblTagName = new Label();
         lblTagValue = new Label();
         SuspendLayout();
         // 
         // lblTagName
         // 
         lblTagName.AutoSize = true;
         lblTagName.Location = new Point(3, 5);
         lblTagName.Name = "lblTagName";
         lblTagName.Size = new Size(60, 15);
         lblTagName.TabIndex = 0;
         lblTagName.Text = "Tag Name";
         lblTagName.TextAlign = ContentAlignment.MiddleLeft;
         // 
         // lblTagValue
         // 
         lblTagValue.Anchor = AnchorStyles.Top | AnchorStyles.Right;
         lblTagValue.AutoSize = true;
         lblTagValue.Location = new Point(229, 5);
         lblTagValue.Name = "lblTagValue";
         lblTagValue.Size = new Size(56, 15);
         lblTagValue.TabIndex = 1;
         lblTagValue.Text = "Tag Value";
         lblTagValue.TextAlign = ContentAlignment.MiddleRight;
         // 
         // ReadingControl
         // 
         AutoScaleDimensions = new SizeF(7F, 15F);
         AutoScaleMode = AutoScaleMode.Font;
         Controls.Add(lblTagValue);
         Controls.Add(lblTagName);
         Name = "ReadingControl";
         Size = new Size(288, 25);
         Load += ReadingControl_Load;
         ResumeLayout(false);
         PerformLayout();
      }

      #endregion

      private Label lblTagName;
      private Label lblTagValue;
   }
}
